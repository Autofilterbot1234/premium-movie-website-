...
async def save_movie(client, message):
    ...
    quality_entry = {"quality": quality, "file_id": file_id, "message_id": message.id}
...
async def start_command_handler(client, message):
    ...
    if len(message.command) > 1:
        ...
        if action_param.startswith("stream_") or action_param.startswith("download_"):
            file_id = action_param.split("_")[1]
            print(f"Action: {'Stream' if 'stream' in action_param else 'Download'}, Extracted File ID: {file_id}")
            
            try:
                # Send processing message
                processing_msg = await message.reply_text("🔄 আপনার অনুরোধ প্রসেস করা হচ্ছে...")
                
                # Forward the original message from channel
                ...
                print(f"Successfully forwarded file {file_id} to {message.chat.id}")
                
            except Exception as e:
                await message.reply_text("⚠️ দুঃখিত, ফাইলটি পাওয়া যায়নি। অনুগ্রহ করে আবার চেষ্টা করুন।")
                print(f"Error forwarding file {file_id}: {str(e)}")
                
        else:
            print(f"Unknown start command parameter format: {action_param}")
            await message.reply_text("স্বাগতম! আমাদের মুভি সংগ্রহ দেখতে ওয়েবসাইট ভিজিট করুন।")
    else:
        print("Received /start command without any parameter.")
        welcome_msg = """
        🎬 **স্বাগতম!** 🍿

        এই বটের মাধ্যমে আপনি আপনার পছন্দের মুভি দেখতে বা ডাউনলোড করতে পারবেন।

        **ব্যবহার পদ্ধতি:**
        1. আমাদের ওয়েবসাইটে যান
        2. আপনার পছন্দের মুভি নির্বাচন করুন
        3. "Watch" বা "Download" বাটনে ক্লিক করুন
        4. স্বয়ংক্রিয়ভাবে এই বটে রিডাইরেক্ট হয়ে যাবেন
        5. ফাইলটি আপনাকে পাঠানো হবে
